...
with visual_button:
    ...
    with tabs[3]:
        ...
        side_chart
        st.subheader("From the above side by side bar chart we can infer that warehouse F has had all packages delivered on time. The other warehouses have almost equal number of packages delivered and not delivered on time. But its pretty amazing to know that warehouse F has the highest packages stored and also delivered with 100 percent success rate. This shows the efficiency of warehosue F")
        st.markdown("""---""")
        ...
        ship_side_chart = alt.Chart(ship_pivoted_df).mark_bar().encode(
            ...
            alt.Color('on_time')).properties(width = 100, height = 600)
        ship_side_chart
        st.subheader("From the above side by side bar chart we can see that all packages through shipment have been delivered on time. Regardless of the large number of packages being transported through ships. It is proved that ships have served 100 percent efficient in delivering packages on time")
